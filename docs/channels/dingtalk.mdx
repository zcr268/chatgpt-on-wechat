...
> Optional: to reply with streaming Markdown cards, request the `Card.Instance.Write` and `Card.Streaming.Write` permissions under **Permission Management**, and add `"dingtalk_card_enabled": true` to `config.json`.
