...
MiniMax-M3 は OpenAI 互換の chat エンドポイントを通じて、テキスト・画像・動画の入力をネイティブにサポートしています。`minimax_api_key` を設定すると、Agent の Vision ツールは画像リクエストを自動的に MiniMax-M3 へルーティングするため、ビジョンモデルを個別に指定する必要はありません。
