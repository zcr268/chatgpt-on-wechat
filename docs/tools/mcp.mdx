...
- **Form or JSON**: fill in the form, or switch to the JSON tab and paste the config an MCP directory gives you, for example `{"mcpServers":{"fetch":{"command":"uvx","args":["mcp-server-fetch"]}}}`. A pasted config can hold several servers at once.
- **Test connection** handshakes with each server and lists its tools. **Save** runs the same check first; if it fails, fix the config or choose **Save anyway** (useful for servers that need authorization after saving).
