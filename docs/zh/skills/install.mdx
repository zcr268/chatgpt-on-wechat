...
也可以在 Web 控制台和桌面客户端的「能力」页面添加技能。点击 **添加技能** 后：

- **从市场安装**：选择 Cow Skill Hub 或 ClawHub 并填写技能名称，或选择 GitHub 并填写仓库 / 目录地址。
- **本地上传**：拖入 `.zip` / `.tar.gz` 压缩包、`SKILL.md` 或整个技能文件夹。

技能会先被获取下来供你预览（`SKILL.md` 内容、文件列表，以及是否会覆盖已安装的同名技能），确认后才会安装。
