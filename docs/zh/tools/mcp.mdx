...
- **表单或 JSON**：可以逐项填写表单，也可以切换到 JSON 标签，直接粘贴 MCP 市场提供的配置，例如 `{"mcpServers":{"fetch":{"command":"uvx","args":["mcp-server-fetch"]}}}`，一次可以粘贴多个。
- **测试连接** 会握手并列出每个服务提供的工具。点击 **保存** 时也会先做同样的校验；校验未通过时，可以修改配置，或选择 **仍然保存**（适用于保存后才能授权的服务）。
