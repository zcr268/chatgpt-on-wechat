...
The Agent is no longer tied to a single directory — you can set up several workspaces and give each session its own. Available in both the Web console and the desktop client:

- **Multiple workspaces**: create or open any directory as a workspace; the documents, web pages, and code the Agent produces are created and managed there.
- **Per-session workspace**: each session is bound to its own workspace, so tasks running side by side stay out of each other's way.
...
- **Shared system resources**: memory, skills, and the knowledge base still live in the system space (`~/cow` by default).

TODO: screenshot

Docs: [Architecture](https://docs.cowagent.ai/intro/architecture)
