const translations: Record<string, Record<string, string>> = {
  zh: {
    ...
    models_cap_chat_fallback_sub: '仅在主模型彻底失败（重试耗尽）后接管，不留死局',
    models_chat_fallback_enable: '启用兜底模型',
    models_chat_fallback_off_hint: '未启用。当前所有请求都由 {model} 处理，失败即报错。',
    models_chat_fallback_off_hint_plain: '未启用。主模型失败时不会切换，错误会直接返回。',
    models_chat_fallback_max_switches: '单轮最大切换次数',
    models_chat_fallback_max_switches_hint: '每轮对话最多切换几次（1-5），防止在故障模型间反复横跳',
  ...
  en: {
    ...
    models_cap_chat_fallback_sub: 'Takes over only after the main model fails for good (retries exhausted)',
    models_chat_fallback_enable: 'Enable the fallback model',
    models_chat_fallback_off_hint: 'Off. Every request goes to {model}; a failure surfaces as an error.',
    models_chat_fallback_off_hint_plain: 'Off. A main-model failure is reported as-is, with no switch.',
    models_chat_fallback_max_switches: 'Max switches per turn',
    models_chat_fallback_max_switches_hint: 'How many times one turn may switch (1-5); guards against ping-pong',
